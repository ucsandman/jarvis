...
$exe=(Resolve-Path ".artifacts/windows-$version/Jarvis-$version-Windows-x64.exe").Path
if(Get-NetTCPConnection -LocalPort 4317 -State Listen -ErrorAction SilentlyContinue){throw 'Quit the running Jarvis before this isolated check.'}
...
try {
    ...
    $signal=[Threading.EventWaitHandle]::OpenExisting("Local\JarvisOpenApp-$($health.instanceId)-paint")
    ...
    $quit=[Threading.EventWaitHandle]::OpenExisting('Local\JarvisDesktopQuit')
    $null=$quit.Set();$quit.Dispose()
    if(-not $launcher.WaitForExit(15000)){throw 'Jarvis did not quit.'}
    foreach($app in $ownedApps){$app.Refresh();if($app.HasExited){throw 'Quitting Jarvis terminated its user application.'}}
    if(Get-NetTCPConnection -LocalPort 4317 -State Listen -ErrorAction SilentlyContinue){throw 'Jarvis left its local server running.'}
    Write-Output "PASS: packaged fixed-app launch; $($ownedApps.Count) owned Paint process survived Jarvis Quit; local server stopped."
} finally {
