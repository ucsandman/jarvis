...
try {
  ...
  const observed = { result:built.result.observation,model:built.model };
  ...
  console.log(JSON.stringify({ phase: 'combined-build', elapsedSeconds:(Date.now()-started)/1000, model: built.model, characters: built.result.html.length, changes: built.result.changes }));
