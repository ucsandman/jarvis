...
try {
  ...
  await page.getByText('Open the prepared task-board sample',{exact:true}).click();
  const frame=page.frameLocator('#demo');
  await frame.getByRole('textbox',{name:'Task title'}).fill('Launch check');
  await frame.getByRole('button',{name:'Add task',exact:true}).click();
  const task=frame.locator('.task').filter({hasText:'Launch check'});
  await task.locator('select').selectOption('done');
  assert.equal(await frame.locator('#done .task').filter({hasText:'Launch check'}).count(),1);
  await frame.getByRole('searchbox').fill('Launch check');
  assert.equal(await frame.locator('.task').count(),1);
  await page.getByRole('button',{name:'Reset sample'}).click();
  await frame.getByRole('searchbox').waitFor();
  await page.waitForFunction(()=>document.getElementById('demo-status').textContent.includes('reset'));
  await frame.getByText('Sketch the first screen',{exact:true}).waitFor();
  assert.equal(await frame.locator('.task').count(),3);
  ...
  assert.ok(await page.getByText('This website provides a prepared walkthrough and static sample.',{exact:false}).isVisible());
  const href=await page.locator('#download-zip').getAttribute('href');
  assert.equal(href,'https://github.com/ucsandman/jarvis/releases/download/v0.3.0/Jarvis-0.3.0-Windows-x64.exe');
  ...
  for(const path of ['/api/session','/server.mjs','/.env']) assert.equal((await page.request.get(`${base}${path}`)).status(),404,path);
  ...
  console.log(`PASS: ${base}; 25 checks: walkthrough tabs and keyboard change, actual sketch and captured before/after evidence, explicit no-inference labels, optional sample add/move/search/reset, FAQ, pinned download, 8 public assets, 3 private routes absent, mobile layout, zero browser errors.`);
