...
try {
  ...
  await page.reload();await ready();assert.equal(await page.locator('#effort-choice').inputValue(),'max');
  ...
  assert.match(await page.locator('#billing-note').innerText(),/Usage credits/);
  assert.equal(requests.length,0);
  await page.locator('#login').click();await ready();
  await page.locator('#direction').fill('Build a tiny board with my selected effort');
  await page.locator('#build').click();assert.match(await page.locator('#consent-detail').innerText(),/paid usage credits/);await page.locator('#accept-consent').click();
  ...
  await page.locator('#effort-choice').selectOption('low');await ready();
  ...
  await page.setViewportSize({width:1440,height:1000});await page.locator('#model-choice').selectOption('fable');
  ...
  console.log('PASS: 13 model/effort browser assertions: preference persistence, effort payload, in-flight lock, Fable login and paid-credit consent before inference, saved version retention, mobile layout and zero page errors. Synthetic inference only.');
