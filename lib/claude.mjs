...
export async function installClaude(signal,selected) {
  ...
  const dir=await mkdtemp(join(tmpdir(),'sidelook-claude-install-'));
  try {
...
export function parseClaudeResult(result,effort,model=DEFAULT_ANTHROPIC) {
  ...
  // Claude reports cached input in two places: what was read from the cache and what was written to it. Both are input the turn did not pay full price for.
  return {result:complete.structured_output,model:chosen.model,effort,provider:'Anthropic subscription',tokens:(complete.usage?.input_tokens||0)+(complete.usage?.output_tokens||0),cachedTokens:(complete.usage?.cache_read_input_tokens||0)+(complete.usage?.cache_creation_input_tokens||0)};
}
...
export async function inferClaude({system,prompt,image,schema,effort,model=DEFAULT_ANTHROPIC},signal,onProgress) {
  ...
  const dir=await mkdtemp(join(tmpdir(),'sidelook-claude-'));
  try {
