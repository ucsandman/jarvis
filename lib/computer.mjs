...
export class Computer {
  ...
  // Jarvis was not already looking at. A launch opens something new, and choosing it stays with the person.
  async observe(action){
    if(action.kind==='launch')return {available:false,summary:'A new window may have opened. Refresh the list and choose it yourself; Jarvis does not inspect it on its own.'};
  ...
  async handle(data,signal){
    ...
    if(op==='read'){
      ...
      try {
        ...
        if(!matches.length) throw new AppError('That window is not open, or Jarvis cannot read it. Bring it to the front and summon Jarvis again.',404);
    ...
    try {
      ...
      if(op==='approve'){
        ...
        // Acceptance and outcome are two different facts. The reading that follows is local and bounded; the model sees it only on the next manual step.
