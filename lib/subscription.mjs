...
export class SubscriptionError extends Error {
  constructor(code) { super(FAILURE_MESSAGES[code] || FAILURE_MESSAGES.REQUEST_FAILED); this.code = code; }
}
export async function codexCommand() {
  const home = homedir();
  const bases = process.platform === 'win32'
    ? [join(home,'AppData','Roaming','npm'),...(process.env.PATH || '').split(delimiter)]
...
export async function subscriptionLogin(signal) {
  const cli = await codexCommand();
...
export async function installCodex(signal) {
