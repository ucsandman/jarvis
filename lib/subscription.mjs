...
import { selection, choice } from './models.mjs';
...
const FAILURE_MESSAGES = {
  ...
  MODEL_UNAVAILABLE:m=>`${m.label} is unavailable for this subscription. Check your model access. Jarvis will not select another model.`,
  INCOMPLETE_OUTPUT:m=>`${m.label} returned an incomplete application. Your saved versions are safe. Try a smaller change.`,
  REQUEST_FAILED:m=>`${m.label} could not finish this request. Your saved versions are safe. Check your subscription and try again.`
};
...
export function inferenceArgs(schemaPath,outputPath,imagePath,instructionsPath,options) {
  const selected=selection(options);const chosen=choice(selected.model);
  if (chosen.provider!=='openai') throw new Error('Codex arguments require an OpenAI model.');
  const args=['exec','--ignore-user-config','--ephemeral','--skip-git-repo-check','--sandbox','read-only',
    '--model',chosen.model,'--color','never','--json','--output-schema',schemaPath,'--output-last-message',outputPath,
...
export async function infer({ system,prompt,image,schema,model,effort },signal,onProgress) {
  ...
  try {
    ...
    return { result:resultJson,model:chosen.model,effort:selected.effort,provider:'OpenAI subscription',tokens:(completed.usage?.input_tokens||0)+(completed.usage?.output_tokens||0) };
  } finally {
