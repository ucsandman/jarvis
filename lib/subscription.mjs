...
export async function infer({ system,prompt,image,schema,model,effort },signal) {
  const selected=selection({model,effort});
  if (selected.model==='fable') return (await import('./claude.mjs')).inferClaude({system,prompt,image,schema,...selected},signal);
