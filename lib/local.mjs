...
export async function ensureLoaded(chosen,signal,{fetchImpl=fetch,run,vram,lms}={}) {
  ...
  const command=lms===undefined?await lmsCommand():lms; // injectable: CI has no LM Studio, so the tests hand in a path
  if(!command || !run) return {loaded:false,context:Number(before?.loaded_context_length) || null};
  if(before?.state==='loaded') await run(command,['unload',chosen.model],{signal});
  ...
  await run(command,['load',chosen.model,'--context-length',String(LOCAL_CONTEXT),...(ratio?['--gpu',ratio]:[]),'--yes'],{signal});
