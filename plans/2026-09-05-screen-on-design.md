...
- Ctrl+Shift+F12 is registered today by the Computer helper only while it is armed. `RegisterHotKey` refuses a second owner, so the shell registers it once at handle creation and forwards it: to the helper's stop path when Computer mode is armed, and to `screen-off` when following. The helper's own registration goes away in the same change, with the native lifecycle check updated.
