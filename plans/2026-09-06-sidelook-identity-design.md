...
- "sidelook", lowercase, Plus Jakarta Sans 700, letter-spacing -0.02em, beside the node, matching "practical systems" on the parent site. The site self-hosts the face for the wordmark only (`site/plus-jakarta-sans-700.woff2`): no Google Fonts request, and the CSP stays `font-src 'self'`. The app has no network font: Segoe UI Semibold at the same size and spacing, in Settings and About only. The panel header keeps no wordmark (DESIGN.md).
