...
- **Name gate.** `scripts/check.mjs` fails on the word Jarvis (case-insensitive) anywhere in `public/`, `site/`, `desktop/`, `scripts/`, `tests/`, `package.json`, README, PRODUCT, DESIGN, CONTRIBUTING, SECURITY and `docs/COMPUTER.md`, `docs/MODELS.md`, `docs/WINDOWS.md`, `docs/SITE.md`, except the one marked line in `Launcher.cs`. `verify:site` adds the old name to the strings it rejects on the built site.
