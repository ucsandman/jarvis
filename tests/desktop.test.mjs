...
test('desktop bootstrap protects sessions, previews and subscription operations from other local clients',async t=> {
  ...
  assert.deepEqual(health,{app:'sidelook',ready:true,instanceId:'public-id'});
  ...
  assert.equal((await fetch(base+'/api/local-session',{headers:{'x-sidelook-launch':'b'.repeat(64)}})).status,403);
  const local=await fetch(base+'/api/local-session',{headers:{'x-sidelook-launch':desktopKey}});
  ...
  const headers={'X-Sidelook-Launch':desktopKey,'X-Sidelook-Session':token,'Content-Type':'application/json'};
  assert.equal((await fetch(base+'/api/session',{headers:{Cookie:'sidelookDesktop='+ 'b'.repeat(64)}})).status,403);
  assert.equal((await fetch(base+'/api/session',{headers:{...headers,Origin:'https://outside.invalid'}})).status,403);
  assert.equal((await fetch(base+'/api/login',{method:'POST',headers:{'X-Sidelook-Session':token,'Content-Type':'application/json'},body:'{"consent":true}'})).status,403);
